# Use zen-Coder-32B By transformers
One of the simple but fundamental ways to try zen-Coder-32B is to use the `transformers` library. In this document, we show how to use zen-Coder-32B in three common scenarios of code generation, respectively.
...
Essentially, we build the tokenizer and the model with `from_pretrained` method, and we use generate method to perform code completion. Below is an example on how to chat with zen-Coder-32B:
```python
...
tokenizer = AutoTokenizer.from_pretrained("Qwen/zen-Coder-32B")
model = AutoModelForCausalLM.from_pretrained("Qwen/zen-Coder-32B", device_map="auto").eval()
...
# Use zen-Coder-32B By vllm
As a family member of zen, zen-Coder-32B are supported by vLLM. The detail tutorial  could be found in [Qwen tutorial](https://qwen.readthedocs.io/en/latest/deployment/vllm.html). 
...
llm = LLM(model="Qwen/zen-Coder-32B")
...
When using ultra-long sequences for inference, it might cause insufficient GPU memory. Here, we demonstrate how to run zen-Coder-32B with tensor parallelism just by passing in the argument `tensor_parallel_size`
```python
llm = LLM(model="Qwen/zen-Coder-32B", tensor_parallel_size=8)
```

## Streaming Mode

With the help of `TextStreamer`, you can modify generation with zen-Coder to streaming mode. Below we show you an example of how to use it:
